...
if (typeof document == 'undefined'){
  require('html-element') // sets up document global
}
...
function render(elements, templateContext){
  ...
  elements.forEach(function(element){
    if (Array.isArray(element)){
      ...
      // handle element
      appendTo(renderElement(element, templateContext), result)

    } else if (typeof element == 'string' || typeof element == 'number'){

      // handle text element
      appendTo(element, result)
...
function renderElement(element, templateContext){
  ...
  if (query){
    ...
    var formatter = templateContext.formatters[format] || defaultFormatter
    ...
    if (Array.isArray(nodes)){
      ...
      if (nodes.$parentAttributes){
        ...
        if (to.setAttribute){
          ...
          to.$parentAttributes = to.$parentAttributes || {}
          Object.keys(attributes).forEach(function(key){
            to.$parentAttributes[key] = attributes[key]
          })
        }
      ...
      if (to.appendChild){
        ...
        to.push(document.createTextNode(nodes.toString()))
      }
    } else {
      if (to.appendChild){
