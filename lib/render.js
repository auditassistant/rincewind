...
var getLocals = require('./get_locals')
  ...
  for (var i=0;i<parts.length;i++){
    ...
    if (typeof part == 'string'){ // default
      ...
      if (part.as){
        ...
        context.override = copy(context.override)
        context.override[part.as] = context.source
      }
      ...
      if (part.v){ // view
        ...
        if (view){
          if (typeof view == 'function'){
            ...
            newContext.views = substituteSelf(view.views)
            newContext.locals = getLocals(view)
...
function substituteSelf(views, self) {
  // deal with circular stringify
  if (views) {
    var result = {}
    for (var k in views) {
      result[k] = views[k] === '$self' ? 
        self : views[k]
    }
    return result
  }
